...
# Files are now in backup directory
for ($i = 0; $i -lt $files.Count; $i++) {
    $inputFile = "$backupDir/" + $files[$i]
    $outputFile = "$baseDir/" + $names[$i]
    
    if (Test-Path $inputFile) {
        Write-Host "Reprocessing $($files[$i]) -> $($names[$i])..."
        # Simplified command: Infers webp from extension. Removed 'toFormat webp'
        cmd /c "npx -y sharp-cli -i `"$inputFile`" -o `"$outputFile`" resize 1000 --withoutEnlargement true"
    }
    else {
        Write-Host "Source file $($files[$i]) not found in backup."
    }
}
